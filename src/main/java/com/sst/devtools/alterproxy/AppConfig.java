...
import com.sst.devtools.alterproxy.LFSMapper.IndexHandlePolicy;
...
public class AppConfig {
    public static final File DEFAULT_CONFIG_FILE =
        new File(System.getProperty("user.home") + "/.sst-devtools-alter-proxy.yml");
    public static final int DEFAULT_LISTENING_PORT = 10088;

    int port = DEFAULT_LISTENING_PORT;
    List<LFSMapEntry4Yaml> listOfMapEntry4Yaml = new ArrayList<>();

    public static AppConfig load(File f) throws IOException {
        Yaml y = new Yaml();
        return y.loadAs(Files.toString(f, StandardCharsets.UTF_8), AppConfig.class);
    }
